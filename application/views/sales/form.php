  ...
  $('.addProduct').on('click', function(e) {
    ...
    if (checkAvailablQty({
      ...
      if (qty !== "" && qty > 0 && Product_ID !== "" && Selling_Price !== "" && HSN !== "" && unit !== "") {
        ...
        $(e.target).closest("tr").find('[name="Purchasing_Quantity[]"]').css({
          "color": "red",
...
function calcSubTotal() {
  ...
  if (payment_array.length) {
    var table =
      "<table class='table' border=1><tr style='background-color: #f5f5f5;'><th>Payment Method</th><th>Amount</th><th>Action</th></tr>";
    for (let i = 0; i < payment_array.length; i++) {
      ...
      table += "<tr><td>" + payment_array[i].Payment_Mode + "</td><td>" + payment_array[i].Add_Payment +
        "</td><td><i style='color:red;cursor:pointer' id='payment_array-" + i + "' onclick='delete_payment(" + i +
        ")' class='icon-trash-alt'></i></td></tr>";
    }
    table +=
      "<tr style='font-weight:600;font-size: 18px;background-color: #f5f5f5;'><td style='text-align:right;'>Total</td><td>" +
      amount_Due_total + "</td><td></td></tr></table>";
    $('#payment_type_list').append(table);
  }
  $('#Hidden_Amount_Due').val(JSON.stringify(payment_array));
  $('#Received_Amount').val(amount_Due_total);
  ...
  if (taxSummary.length) {
    var taxSummarytable =
      "<table class='table' border=1><tr style='background:#f5f5f5'><th>GST</th><th>Price</th><th>IGST</th><th>CGST</th><th>SGST</th></tr>";

    for (let i = 0; i < taxSummary.length; i++) {
      totalIGST += taxSummary[i].IGST;
      totalCGST += taxSummary[i].CGST;
      totalSGST += taxSummary[i].SGST;
      taxSummarytable += "<tr><td>" + taxSummary[i].GST + "%</td><td>" + taxSummary[i].subTotal + "</td><td>" +
        taxSummary[i].IGST + "</td><td>" + taxSummary[i].CGST + "</td><td>" + taxSummary[i].SGST + "</td></tr>";
    }

    var totalTaxAmount = eval(totalIGST + totalCGST + totalSGST).toFixed(2);

    taxSummarytable +=
      "<tr style='font-weight:600;font-size: 18px;background-color: #f5f5f5;'><td style='text-align:right;' colspan=2>Total</td><td colspan=3>" +
      totalTaxAmount + "</td></tr></table>";
...
?>
            ...
            <div class="form-group">
              ...
              <div class="col-lg-9">
                <input style="background: #f5f5f5;cursor:not-allowed" readonly name="Customer_Address"
                  id="Customer_Address" value="<?php echo $formData ? $formData['Customer_Address'] : "" ?>" type="text"
                  class="form-control">
              </div>
            </div>
            <div class="form-group">
              ...
              <div class="col-lg-9">
                <input style="background: #f5f5f5;cursor:not-allowed" readonly name="Customer_GSTIN" id="Customer_GSTIN"
                  value="<?php echo $formData ? $formData['Customer_GSTIN'] : "" ?>" type="text" class="form-control">
              </div>
            </div>
            <div class="form-group">
              <label class="col-lg-3 control-label">State : </label>
              <div class="col-lg-9">
                <input style="background: #f5f5f5;cursor:not-allowed" readonly name="State" id="State"
                  value="<?php echo $formData ? $formData['State'] : "" ?>" type="text" class="form-control">
              </div>
            </div>
          </fieldset>
        </div>
        <div class="col-md-6">
          <fieldset>
            ...
            <div class="form-group">
              ...
              <div class="col-lg-9">
                <input style="background: #f5f5f5;cursor:not-allowed" name="Sales_Invoice" readonly
                  value="<?php echo $formData ? $formData['Sales_Invoice'] : $lastSalesId ?>" type="text"
                  class="form-control " placeholder="Enter Sales Invoice Number">
              </div>
            </div>
            ...
            <div class="form-group">
              ...
              <div class="col-lg-9">
                <input name="Sales_Notes" autocomplete="off"
                  value="<?php echo $formData ? $formData['Sales_Notes'] : "" ?>" type="text" class="form-control "
      ...
      <div class="row">
        <div class="col-sm-12">
          <div class="">
            <table class="panel panel-flat table products_list product_details" id="product_details"
              style="margin:15px 0">
              <thead>
                <tr>
                  <th>Product</th>
...
?>
                  ...
                  <td>
                    <div class="form-group">
                      <input type="number" autocomplete="off" onchange="return checkAvailablQty(this)"
                        name="Purchasing_Quantity[]" placeholder="0" class="form-control quantity">
                    </div>
                  </td>
                  ...
                  <td style="text-align: center;">
                    <div class="form-group">
                      <a href="#" class="btn btn-success addProduct pull-right mb-20"><span class="icon-plus3"></span>
                        Add Product</a>
                    </div>
                  </td>
                </tr>
                ...
                <table class="table ">
                  <tr>
                    ...
                    <td>
                      ...
                      <input style="width:50px" type="text" readonly name="Sub_Total" id="sub_total"
                        class="form-control" value="<?php echo $sub_total;?>" placeholder="Sub total" />
                    </td>
                  </tr>
                  <tr>
                    <td>GST</td>
                    <td>
                      <table>
                        <tr>
                          <td>CGST &nbsp;: </td>
                          <td><input type="text" readonly name="CGST" id="CGST" class="form-control" value=""
                              placeholder="CGST" /></td>
                        </tr>
                        <tr>
                          <td>SGST &nbsp;: </td>
                          <td><input type="text" readonly name="SGST" id="SGST" class="form-control" value=""
                              placeholder="SGST" /></td>
                        </tr>
                        <tr>
                          <td>IGST &nbsp;: </td>
                          <td><input type="text" readonly name="IGST" id="IGST" class="form-control" value=""
                              placeholder="IGST" /></td>
                        </tr>
                      </table>
                    </td>
                  ...
                  <tr>
                    <td>Discount</td>
                    <td><input type="text" name="Discount" onkeyup="calcSubTotal()" id="final_discount"
                        autocomplete="off" value="0" class="form-control" placeholder="Discount" /></td>
                  </tr>
                  <tr>
                    <td><strong>Grand Total</strong></td>
                    <td><input type="text" style="font-size: 22px;font-weight: 500;" name="Grand_Total" readonly
                        id="grand_total" class="form-control" value="0" placeholder="Grand Total" /></td>
                  </tr>
                  <tr>
                    <td>Amount Due</td>
                    <td><input type="text" readonly name="Amount_Due" id="Amount_Due" autocomplete="off" value="0"
                        class="form-control" placeholder="Amount_Due" />
                      <input type="hidden" readonly name="Hidden_Amount_Due" id="Hidden_Amount_Due" />
                      <input type="hidden" readonly name="Received_Amount" id="Received_Amount" />
                    </td>
                  </tr>
                  <tr>
                    <td>Add Payment</td>
      ...
      <div class="text-right mt-20">
        ...
        <button name="save" value="save" type="submit"
          class="btn bg-teal-400 access-multiple-open legitRipple"><?php echo $btnName; ?> <i
            class="icon-circle-right2 position-right"></i></button>
        <button name="save_print" value="save_print" type="submit" class="btn btn-success legitRipple legitRipple">Save
          & Print <i class="icon-printer2 position-right"></i></button>
      </div>
    </div>
  </div>
</form>